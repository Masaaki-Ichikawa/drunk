<x-app-layout>    
    ...
    <form class="flex flex-col items-center" action="{{ route('recipe_edit_upload', ['id' => $recipe->id]) }}" method="post" enctype="multipart/form-data">
        ...
        @endif
        <input class="border-gray-300 rounded" name="name" type="text" value="{{ $recipe->name }}">
        ...
        <img class="w-1/2 h-1/2" src="{{ asset($recipe->image_path) }}" alt="">
        <p class="pt-6">↓</p>

        <div class="w-1/2 h-1/2 pt-6" id="preview_edit"></div>
        ...
        @endif
        <textarea id="recipe" class="border-gray-300 resize-none rounded" name="recipe" cols="30" rows="20">{{ $recipe->recipe }}</textarea>
